...
@Injectable()
export class SqliteGetReservationWithClient
  extends SqliteBaseClass
  implements GetReservationWithClientDAO
{
  async get(id: string): Promise<{ client: Client; reservation: Reservation }> {
    ...
    return {
      client: Client.create(
        ...
        Timestamps.restore(
          new Date(result.clientCreated),
          new Date(result.clientUpdated),
        ),
        SoftDelete.empty(),
      ),
      reservation: Reservation.create(
        ...
        Timestamps.restore(
          new Date(result.resCreated),
          new Date(result.resUpdated),
        ),
        SoftDelete.empty(),
      ),
    };
