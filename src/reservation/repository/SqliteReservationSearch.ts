...
import {ReservationSearcherDAO} from "../../../core/Reservation/Model/DAO/ReservationSearcherDAO";
...
@Injectable()
export class SqliteReservationSearch extends SqliteBaseClass implements ReservationSearcherDAO {
    async search(query: ReservationSearchQuery): Promise<ReservationResponse[]> {
        ...
        const sql = `
            ...
            WHERE
                r.deleted_at IS NULL AND ss.seasonId = @id
                ${query.state !== 'All' ? 'AND r.state = @state' : ''}
            ORDER BY r.created_at ${query.direction!.toUpperCase()}
